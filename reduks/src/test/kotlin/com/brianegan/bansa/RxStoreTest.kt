...
class RxStoreTest {
    ...
    fun subscribers_should_be_notified_when_the_state_changes() {
        ...
        store.subscribe(subscriber1, observeOnAndroidMainThread = false) //false, otherwise will throw exception in test
        store.subscribe(subscriber2, observeOnAndroidMainThread = false) // false, otherwise will throw exception in test
    ...
    fun the_store_should_not_notify_unsubscribed_objects() {
        ...
        store.subscribe(subscriber1, observeOnAndroidMainThread = false) //false, otherwise will throw exception in test
        val subscription = store.subscribe(subscriber2, observeOnAndroidMainThread = false) // false, otherwise will throw exception in test
